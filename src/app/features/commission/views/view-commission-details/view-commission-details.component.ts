...
export class ViewCommissionDetailsComponent extends BaseViewComponent
  ...
  initValidator(): void {
    const initialRoleValidator = this.commissionValidationService.getRoleValidator();

    if (!isNil(this.currentBookmark.viewState.commissionValidator)) {
      initialRoleValidator.initValidatorResultStates(this.currentBookmark.viewState.commissionValidator);
    }

    this.currentBookmark.viewState.commissionValidator = initialRoleValidator;
