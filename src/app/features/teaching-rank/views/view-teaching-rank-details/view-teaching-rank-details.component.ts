...
import {TeachingRankMapperService} from '../../services/teaching-rank-mapper.service';
import {TeachingRankFacadeService} from '../../services/teaching-rank-facade.service';
import {TeachingRankValidationService} from '../../services/teaching-rank-validation.service';
...
export class ViewTeachingRankDetailsComponent extends BaseViewComponent
  ...
  public viewState: ITeachingRankDetailsViewState;
  ...
  constructor(
    ...
    protected teachingRankValidationService: TeachingRankValidationService,
    ...
    super({
      nameTranslateKey: 'COMMON.BOOKMARK.TEACHING_RANK.DETAILS.BOOKMARK_NAME',
      descriptionTranslateKey: 'COMMON.BOOKMARK.TEACHING_RANK.DETAILS.BOOKMARK_DESCRIPTION',
      iconSvg: BookmarkIcon.teachingRankDetails,
    ...
    if (this.document.location.pathname.endsWith('teaching-rank/new')) {
      this.isNew = true;
    } else {
      this.teachingRankId = Number(this.route.snapshot.paramMap.get('id'));
    }

    this.teachingRankFacadeService.getTeachingRankDetailsViewState$()
  ...
  checkDataChanged(): boolean {
    return !isEqual(this.currentBookmark.data.teachingRankDetail, this.currentBookmark.data.teachingRankDetailCopy);
  }

  createData(): void {
    this.validator.validateDto(this.teachingRank);
    if (this.validator.dtoValidationResult.isValid) {
      this.teachingRankFacadeService.createTeachingRank$(this.teachingRank)
        .subscribe(value => {
          this.customNotificationService.showSuccess(this.translateService.instant('TEACHING_RANK.DETAILS.NOTIFICATION.SUCCESS_CREATE'));
    ...
    deleteDialog.result.subscribe((result) => {
      if (result instanceof DialogCloseResult) {
        ...
        if (result.text === this.translateService.instant('COMMON.BUTTON.YES')) {
          this.teachingRankFacadeService.deleteTeachingRank$(this.teachingRank)
            .pipe(takeUntil(this.onDestroy))
            .subscribe(() => {
              this.bookmarkService.getCurrentDataItem().teachingRankDetail.isDeleted = true;
              this.bookmarkService.getCurrentDataItem().teachingRankDetailCopy.isDeleted = true;
              this.refreshTitleHeaderButtons();
              this.customNotificationService.showSuccess(this.translateService
                .instant('TEACHING_RANK.DETAILS.NOTIFICATION.SUCCESS_DELETE'));
            }, error => {
              const errors = this.errorService.getMessagesToShow(error.errors);
  ...
  getData(): void {
    if (this.isNew && isNil(this.currentBookmark.data.teachingRankDetail)) {
  ...
  navigateToDataPageAfterCreating(id: number): any {
    this.bookmarkService.deleteBookmarkById(this.bookmarkService.getCurrentId()).subscribe(_ => {
      const route = `teaching-rank/details/${id}`;
      return this.router.navigate([route]);
    });
  }

  setData(value: ITeachingRankViewModel): void {
    this.validator.setDto(value);
    this.teachingRank = value;
    this.changeTitle();
    this.refreshTitleHeaderButtons();
  }
  ...
  updateData(): void {
    ...
    if (this.validator.dtoValidationResult.isValid) {
      this.teachingRankFacadeService.updateTeachingRank$(this.teachingRank)
        .pipe(takeUntil(this.onDestroy))
        .subscribe(value => {
          this.customNotificationService.showSuccess(this.translateService
            .instant('TEACHING_RANK.DETAILS.NOTIFICATION.SUCCESS_UPDATE'));
          this.setData(value);
        }, error => {
          const errors = this.errorService.getMessagesToShow(error.errors);
  ...
  initValidator(): void {
    const initialTeachingRankValidator = this.teachingRankValidationService.getTeachingRankValidator();

    if (!isNil(this.currentBookmark.viewState.teachingRankValidator)) {
      initialTeachingRankValidator.initValidatorResultStates(this.currentBookmark.viewState.teachingRankValidator);
    }

    this.currentBookmark.viewState.teachingRankValidator = initialTeachingRankValidator;
    this.validator = this.currentBookmark.viewState.teachingRankValidator;
  }
  ...
  changeTitle(): void {
    this.titleValue = this.teachingRank.name || (this.isNew ? this.translateService.instant('COMMON.NEW') : '');
    this.currentBookmarkTask.nameValue = this.titleValue;
  }

  openNewData(): Promise<boolean> {
    const route = `/teaching-rank/new`;
    return this.router.navigate([route]);
  }
  ...
  cancelRestore(): void {
    this.teachingRank = cloneDeep(this.bookmarkService.getCurrentDataItem().teachingRankDetailCopy);
    this.bookmarkService.getCurrentDataItem().teachingRankDetail = this.teachingRank;
  ...
  refreshTitleHeaderButtons(): void {
    this.titleHeaderButtonManager.getById('delete')
      .setVisibility(!this.teachingRank.isDeleted && !this.isNew && writeRoles.includes(this.authService.currentRole));
    ...
    this.titleHeaderButtonManager.getById('update')
      .setVisibility(!this.isNew && !this.teachingRank.isDeleted && writeRoles.includes(this.authService.currentRole));
    ...
    this.titleHeaderButtonManager.getById('delete-status')
      .setVisibility(!this.isNew && this.teachingRank.isDeleted);

    this.titleHeaderButtonManager.getById('restore')
      .setVisibility(!this.isNew && !this.viewState.restoring
        && this.teachingRank.isDeleted && writeRoles.includes(this.authService.currentRole));

    this.titleHeaderButtonManager.getById('confirm-restore')
      .setVisibility(!this.isNew && this.teachingRank.isDeleted && this.viewState.restoring
  ...
  onTitleButtonClick(clickedButton: TitleHeaderElement) {
    switch (clickedButton.id) {
      case 'refresh':
        this.teachingRankFacadeService.refreshDetails$.next();
        break;
