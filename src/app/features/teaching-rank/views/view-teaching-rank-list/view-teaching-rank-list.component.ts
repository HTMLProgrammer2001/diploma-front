...
@Component({
  selector: 'cr-view-teaching-rank-list',
  templateUrl: './view-teaching-rank-list.component.html',
})
export class ViewTeachingRankListComponent extends BaseViewComponent implements OnInit, OnDestroy {
  public dataSource: IPaginator<ITeachingRankListViewModel>;
  ...
  public filter: ITeachingRankFilterViewModel;
  private cacheInitialized: boolean;
  private onDestroy = new ReplaySubject(1);

  private deletedColumn: IEditGridColumnSetting = {
    field: 'isDeleted',
    titleTranslateKey: 'TEACHING_RANK.LIST.GRID.DELETED',
    type: 'boolean',
    autoFit: true,
    hidden: true,
    sortable: false
  ...
  public columnSettings: Array<IEditGridColumnSetting> = [
    {
      field: 'id',
      titleTranslateKey: 'TEACHING_RANK.LIST.GRID.ID',
      type: 'link',
      disabledIf: () => !readRoles.includes(this.authService.currentRole)
    },
    {
      field: 'name',
      titleTranslateKey: 'TEACHING_RANK.LIST.GRID.NAME',
      type: 'text',
    },
    this.deletedColumn,
  ];
  ...
  constructor(
    ...
    this.cacheInitialized = !isNil(this.bookmarkService.getCurrentViewState().teachingRankFilter);
    this.teachingRankFacadeService.getViewStateTeachingRankFilter$()
  ...
  getDataList(): void {
    forkJoin({paginator: this.teachingRankFacadeService.getViewStateTeachingRankListPaginator$()}).pipe(
      takeUntil(this.onDestroy),
      switchMap(values => {
        this.paginator = values.paginator;
        return this.teachingRankFacadeService.getTeachingRankList$(this.paginator, this.filter);
      })
  ...
  loadDataList() {
    this.teachingRankFacadeService.loadTeachingRankList$(this.paginator, this.filter).subscribe(value => {
  ...
  cellClick(event: ITeachingRankListViewModel): Promise<boolean> {
    const route = `teaching-rank/details/${event.id}`;
    return this.router.navigate([route]);
  }
