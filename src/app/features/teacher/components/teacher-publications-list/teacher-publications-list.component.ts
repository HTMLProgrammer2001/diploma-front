...
@Component({
  selector: 'cr-teacher-publications-list',
  templateUrl: './teacher-publications-list.component.html',
})
export class TeacherPublicationsListComponent implements OnInit, OnDestroy, OnChanges {
  @Input() teacher: ITeacherViewModel;

  public dataSource: IPaginator<ITeacherPublicationListViewModel>;
  ...
  public columnSettings: Array<IEditGridColumnSetting> = [
    {
      field: 'id',
      titleTranslateKey: 'TEACHER.DETAILS.PUBLICATIONS_LIST.GRID.ID',
      type: 'link',
      disabledIf: () => !readRoles.includes(this.authService.currentRole)
    },
    {
      field: 'title',
      titleTranslateKey: 'TEACHER.DETAILS.PUBLICATIONS_LIST.GRID.TITLE',
      type: 'text'
    },
    {
      field: 'date',
      titleTranslateKey: 'TEACHER.DETAILS.PUBLICATIONS_LIST.GRID.DATE',
      type: 'date'
    },
  ];
  ...
  getDataList(): void {
    forkJoin({paginator: this.teacherFacadeService.getViewStateTeacherPublicationListPaginator$()}).pipe(
      takeUntil(this.onDestroy),
      switchMap(values => {
        this.paginator = values.paginator;
        return this.teacherFacadeService.getTeacherPublicationList$(this.paginator, this.teacher);
      })
  ...
  loadDataList() {
    this.teacherFacadeService.loadTeacherPublicationList$(this.paginator, this.teacher)
      .pipe(takeUntil(this.onDestroy))
      .subscribe(value => {
