...
@Component({
  selector: 'cr-teacher-honors-list',
  templateUrl: './teacher-honors-list.component.html',
})
export class TeacherHonorsListComponent implements OnInit, OnDestroy, OnChanges {
  ...
  public columnSettings: Array<IEditGridColumnSetting> = [
    {
      field: 'id',
      titleTranslateKey: 'TEACHER.DETAILS.HONORS_LIST.GRID.ID',
      type: 'link',
      disabledIf: () => !readRoles.includes(this.authService.currentRole)
    },
    {
      field: 'title',
      titleTranslateKey: 'TEACHER.DETAILS.HONORS_LIST.GRID.TITLE',
      type: 'text'
    },
    {
      field: 'orderNumber',
      titleTranslateKey: 'TEACHER.DETAILS.HONORS_LIST.GRID.ORDER_NUMBER',
      type: 'text'
    },
    {
      field: 'date',
      titleTranslateKey: 'TEACHER.DETAILS.HONORS_LIST.GRID.DATE',
      type: 'date'
    },
    {
      field: 'isActive',
      titleTranslateKey: 'TEACHER.DETAILS.HONORS_LIST.GRID.IS_ACTIVE',
      type: 'boolean'
    },
  ];
  ...
  getDataList(): void {
    forkJoin({paginator: this.teacherFacadeService.getViewStateTeacherHonorListPaginator$()}).pipe(
      takeUntil(this.onDestroy),
      switchMap(values => {
        this.paginator = values.paginator;
        return this.teacherFacadeService.getTeacherHonorList$(this.paginator, this.teacher);
      })
  ...
  loadDataList() {
    this.teacherFacadeService.loadTeacherHonorList$(this.paginator, this.teacher)
      .pipe(takeUntil(this.onDestroy))
      .subscribe(value => {
